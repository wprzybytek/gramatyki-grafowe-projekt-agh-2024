...
def big_graph() -> nx.Graph:
    ...
    G.add_nodes_from([
        ...
        ('Q:1.25:0.75', {'label': 'Q', 'R': 0}),
        ('Q:1.25:0.25', {'label': 'Q', 'R': 1}),
        ('Q:1.75:0.25', {'label': 'Q', 'R': 0}),
        ('Q:1.75:0.75', {'label': 'Q', 'R': 0}),
...
def test_two_graphs_to_cover_p8():
    ...
    assert graph.nodes['Q:1.25:0.75']['R'] == 0, "Node Q:1.25:0.75 should have R=1 before applying the production"
    assert graph.nodes['Q:1.25:0.25']['R'] == 1, "Node Q:1.25:0.25 should have R=1 before applying the production"
    assert graph.nodes['Q:1.75:0.75']['R'] == 0, "Node Q:1.75:0.75 should have R=1 before applying the production"
    assert graph.nodes['Q:1.75:0.25']['R'] == 0, "Node Q:1.75:0.25 should have R=1 before applying the production"
    ...
    production.apply()
    if view:
        plot_graph(graph, "TEST 6 - AFTER PRODUCTION")
