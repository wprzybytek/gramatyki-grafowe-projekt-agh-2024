...
def graph_with_two_sub_graphs_to_cover() -> nx.Graph:
    G = nx.Graph()
...
def test_two_graphs_to_cover_p8():
    graph = graph_with_two_sub_graphs_to_cover()
    production = ProductionP8(graph)
    if view:
        plot_graph(graph, "TEST 3")
    ...
    production.apply()
    if view:
        plot_graph(graph, "TEST 3 - AFTER PRODUCTION")
